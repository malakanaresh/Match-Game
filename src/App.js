...
class App extends Component {
  ...
  getDemandList = () => {
    const {newTabId} = this.state

    const demandImgList = imagesList.filter(
      eachOne => eachOne.category === newTabId,
    )
    return demandImgList
  }

  onImgChange = id => {
    const {chImg} = this.state
    const l = imagesList.length
    const one = Math.ceil(Math.random() * l)
    this.setState({chImg: imagesList[one]})
  ...
  render() {
    const {newTabId, chImg, count, timmer, isTrue} = this.state

    const filterdemandImgList = this.getDemandList()
      ...
      <div>
        <div className="container">
          ...
          {isTrue ? (
            <div>
              <div>
                <img src={chImg.imageUrl} alt="match" className="orange" />
              </div>
              <ul className="tab-container">
                {tabsList.map(category => (
                  <TabList
                    key={category.tabId}
                    nCategory={category}
                    onDemandList={this.onDemandList}
                  />
                ))}
              </ul>

              <ul className="down-list">
                {filterdemandImgList.map(eachImage => (
                  <ListItem
                    key={eachImage.id}
                    imgeDetails={eachImage}
